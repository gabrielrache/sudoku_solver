...
def main ():
    ...
    while fl_continuar:
        ...
        if settings.fl_verbose: print (f"\nTurno {qtTurnos}")
        ...
        for l in range (9):
            for c in range (9):        
                ...
                if resolvidoParcial[l,c] == 0:
                    ...
                    sugestao = sudoku.TestaPossibilidades (resolvidoParcial,l,c)  

                    if settings.fl_verbose: print (f"Lista sugestão: {sugestao}")
        
                    ##DEBUG
                    if settings.fl_debug: print(f"matrizSugestao = {matrizSugestao}")

                    # Lógica passa para o jogo se for única sugestão disponível
                    if len(sugestao) == 1: 
                        resolvidoParcial[l,c] = (sugestao[0])   
                        if settings.fl_verbose: print (f"A posição {constants.modeloSestiSudoku[l,c]} recebeu {sugestao[0]} agora!")    
                    else:
                        for x in sugestao:
                            matrizSugestao[l,c,x-1] = True

                            if settings.fl_verbose: print(f"A posição {constants.modeloSestiSudoku[l,c]} pode ser {x}")

                    if settings.fl_debug: print(matrizSugestao[0,:,:])
        ...
        if (settings.fl_verbose and faltantes == 1): print(f"\nAinda falta 1 número para encontrar\n")
        elif settings.fl_verbose: print(f"\nAinda faltam {faltantes} números para serem encontrados!\n")

        if settings.fl_verbose: print(f"Progresso atual do Quebra-cabeça:\n\n{resolvidoFinal}")
